...
void MqttClient::poll()
{
  ...
  while (clientAvailable())
  {
    ...
    if (_rxState == MQTT_CLIENT_RX_STATE_READ_TYPE)
    {
      _rxType = (b >> 4);
      _rxFlags = (b & 0x0f);
      _rxLength = 0;
    ...
    if (_rxState == MQTT_CLIENT_RX_STATE_READ_REMAINING_LENGTH)
    {
    ...
    if (_rxState == MQTT_CLIENT_RX_STATE_READ_VARIABLE_HEADER)
    {
    ...
    if (_rxState == MQTT_CLIENT_RX_STATE_READ_PUBLISH_TOPIC_LENGTH)
    {
      _rxMessageBuffer[_rxMessageIndex++] = b;
    ...
    if (_rxState == MQTT_CLIENT_RX_STATE_READ_PUBLISH_TOPIC)
    {
    ...
    if (_rxState == MQTT_CLIENT_RX_STATE_READ_PUBLISH_PACKET_ID)
    {
      _rxMessageBuffer[_rxMessageIndex++] = b;
    ...
    if (_rxState == MQTT_CLIENT_RX_STATE_DISCARD_PUBLISH_PAYLOAD)
    {
      if (_rxLength > 0)
      {
        _rxLength--;
